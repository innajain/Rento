...
const bookingCard =[
  {
    ...
    description:"Choose and pick from a plethora of verified student Flats & PGs",
    icon:Magnifier
  },
  {
    heading:"Fill in your details",
    description:"Fill in all your necessary personal details required for the booking.",
    icon:FileCheck,
  },
  {
...
},
{
  ...
  return (
    <div className="h-screen flex flex-col gap-6 mt-2">
        <ScrollableContainer>
        </ScrollableContainer>
        <ScrollableContainer>
        </ScrollableContainer>
        <CardsContainer className="bg-landingPage-gray-light">
            {cards.map((card,index)=>{
                return (
                <Card key={index}>
                <CardBody className="flex items-center flex-col gap-6 py-4" >
        ...
        <WhiteContainer>
          <HeadingDiv>
          <Heading>Popular areas for <HighLightedText>students</HighLightedText> in Delhi</Heading>
          <SubHeading>Book student accommodations near universities around Delhi</SubHeading>
          </HeadingDiv>
          <div className="flex flex-col gap-4 ">
              {campuses.map((campus, index) => {
                  return (
                  <div key={index} className="flex p-2 py-4 rounded-lg flex-col gap-4 bg-primary">
                      <span className="text-2xl text-white text-center font-bold">{campus.campus}</span>
                      <div className="flex justify-evenly">
                          {campus.images.map((image, index) => {
                              return (
                              <div key={index} className="flex flex-col gap-2">
                                 {index!==4 ? <Image src={image.image} alt="image" width={200} height={200}/> :
                                 <Image className="rounded-2xl border-[10px]"  src={image.image} alt="image" width={200} height={200}/>
                                 }
                              </div>
                              )
                          })}
        ...
        <WhiteContainer>
          <HeadingDiv>
          <Heading>Book your favourite room in <HighLightedText>3 simple steps</HighLightedText></Heading>
          <SubHeading>What do our 10k+ Students have to say about their experience with ROR</SubHeading>
          </HeadingDiv>
         <div className="flex gap-4">
              {bookingCard.map((card, index) => {
                  ...
                  <div className="flex gap-4 items-center " key={index}>
                    ...
                    {index!==2&& <Image src={Arrow} alt="arrow" width={100} height={1}/>}
                  </div>
                  )
              })}
         </div>
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-12">
              {assistance.map((assist, index) => {
                  return (
                          <div key={index} className="flex items-center border rounded-xl p-4 px-12 flex-col gap-4" >
